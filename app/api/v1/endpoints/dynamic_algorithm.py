...
def create_shift_split_schedule_entries(start_time: datetime, operation: Operation, quantity: int, 
                                       has_production_logs: bool = False) -> List[Dict[str, Any]]:
    """
    Create multiple schedule entries for operations that span across shifts
    This matches the main scheduling algorithm logic exactly
    """
    from app.algorithm.scheduling import get_shift_end, get_next_shift_start
    ...
    while remaining_minutes > 0:
        ...
        # Debug logging
        print(f"    Segment calculation: ratio={segment_ratio:.3f}, pieces_for_segment={pieces_for_this_segment:.1f}, remaining_pieces={remaining_pieces}, cumulative={cumulative_pieces}->{new_cumulative}, segment_pieces={segment_pieces}")
...
def calculate_shift_aware_duration(start_time: datetime, operation: Operation, quantity: int, 
                                 has_production_logs: bool = False) -> Tuple[datetime, timedelta]:
    """
    ...
    """
    schedule_entries = create_shift_split_schedule_entries(start_time, operation, quantity, has_production_logs)
...
def find_last_available_operation(operations: List[Operation], current_time: datetime) -> int:
        ...
        with db_session:
            ...
            for i in range(current_op_index, len(all_operations)):
                ...
                if delete_existing_reschedules.count() > 0:
                    print(f"  Deleting {delete_existing_reschedules.count()} existing rescheduled items for operation {op.id}, order {order.id}")
                ...
                if i == current_op_index:
                    if remaining_qty == 0: # If current op has no remaining quantity, it's completed, skip to next op
                        print(f"Operation {i+1}: Completed in logs, skipping reschedule. Starting next op at {current_time}")
                        continue
                    # For the current operation with remaining work, use remaining quantity
                    quantity = remaining_qty
                    operation_has_logs_local = has_production_logs
                    print(f"Operation {i+1}: Using remaining quantity {quantity} (has_logs={operation_has_logs_local})")
                else:
                    ...
                    for log in ProductionLog.select(lambda pl: pl.operation == op):
                        ...
                        if getattr(log, 'end_time', None) and (latest_end_for_op is None or log.end_time > latest_end_for_op):
                    ...
                    if completed_qty_for_op > 0 or latest_end_for_op is not None:
                        ...
                        if latest_end_for_op and latest_end_for_op > current_time:
                            ...
                            print(f"Operation {i+1}: Anchoring to its own latest log end {current_time} (was {current_time})")
                        quantity = max(0, total_qty - completed_qty_for_op)
                        print(f"Operation {i+1}: Detected existing logs (completed={completed_qty_for_op}/{total_qty}), remaining={quantity}")
                    else:
                ...
                if not operation_has_logs_local:
                    ...
                    if setup_entries:
                        for segment_idx, segment in enumerate(setup_entries):
                            if not _rescheduled_item_exists(order, op, segment['start_time'], segment['end_time'], 'reschedule', 'setup'):
                                RescheduledItem(
                                    order=order,
                                    operation=op,
                                    machine=op.machine,
                                ...
                                print(f"  Created SETUP entry segment {segment_idx + 1} for {op.operation_description}: {segment['start_time']} -> {segment['end_time']} (qty=1)")
                                setup_created = True
                            else:
                                print(f"  Skipped duplicate SETUP entry segment {segment_idx + 1} for {op.operation_description}: {segment['start_time']} -> {segment['end_time']}")
                    ...
                    print(f"  Setup already covered by production logs or handled in initial setup, skipping setup creation for {op.operation_description}")
                ...
                schedule_entries = create_shift_split_schedule_entries(current_time, op, quantity, exclude_setup)
                ...
                print(f"Operation {op.operation_description}: {current_time} -> {schedule_entries[-1]['end_time']} (qty: {quantity})")
                print(f"  Setup: {setup_time}min, Cycle: {cycle_time}min/piece, Total: {total_minutes_for_debug}min") # Used total_minutes_for_debug
                ...
                for segment_idx, segment in enumerate(schedule_entries):
                    ...
                    print(f"  Segment {segment_idx + 1}: {segment['start_time']} -> {segment['end_time']} (qty: {segment['quantity']})")
    ...
    try:
        with db_session:
                ...
                try:
                    ...
                    if schedule_item and getattr(schedule_item, 'order', None):
                        if schedule_item.order == operation.order:
                            ...
                            print(f"Skipping log {getattr(log, 'id', 'unknown')} due to order mismatch: schedule_item.order != operation.order")
                            continue
                    else:
                    ...
                    try:
                        ...
                        if schedule_item_operation is not None and schedule_item_operation != operation:
                            print(f"Skipping log {getattr(log, 'id', 'unknown')} due to operation mismatch: schedule_item.operation != log.operation")
                            continue
                    except Exception:
                        pass
            ...
            for key, agg in aggregated.items():
                ...
                # Debug logging
                print(f"Found production log: Order {agg['order_id']}, Operation {agg['operation_id']}, Part {agg['part_number']}, Completed {agg['completed_qty']}/{agg['total_qty']}, Remaining {remaining_qty}")
...
def process_reschedule_triggers(production_logs: List[Dict[str, Any]]) -> Tuple[int, int, set]: # Modified return type
    ...
    for mid, logs in logs_by_machine.items():
        ...
        try:
            if order_priority_map and enriched:
                ...
                higher_no_log_priorities = sorted({p for p in order_priority_map.values() if p < lowest_logged_priority})
                if higher_no_log_priorities:
                    ...
                    for prio in higher_no_log_priorities:
                        ...
                        if not allowlist:
                            continue
                        print(f"  Pre-cascading higher-priority no-log orders on machine {mid} with priority {prio} from {machine_anchor}")
                        ...
                        with db_session:
                            latest_on_machine = select(ri for ri in RescheduledItem if ri.status == 'reschedule' and ri.machine.id == mid) \
        ...
        for rank, (priority, log_info) in enumerate(enriched, start=1):
            ...
            if int(log_info.get('remaining_qty', 0)) == 0:
                print(f"     Remaining qty is 0 for order {log_info['order_id']}. Finding next operation to reschedule from {anchor}")
                    ...
                    with db_session:
                        ...
                        for i in range(current_op_index + 1, len(all_operations)):
                            ...
                            if remaining_qty > 0:
                                ...
                                print(f"     Found next operation {op.operation_description} with remaining qty {remaining_qty}")
                                break
                            ...
                            with db_session:
                                ...
                                if last_operation:
                                    ...
                                    last_rescheduled = select(ri for ri in RescheduledItem 
                                                           ...
                                                           ri.status == 'reschedule').order_by(lambda ri: desc(ri.end_time)).first()
                                    if last_rescheduled:
                                        machine_anchor = last_rescheduled.end_time
                                        anchor = machine_anchor
                                        print(f"     Using last operation {last_operation.operation_description} end time: {machine_anchor}")
                                    else:
                ...
                try:
                    ...
                    allowlist_filtered = {oid for oid in allowlist if (log_info['operation_id'], oid) not in processed_keys}
            ...
            if order_priority_map:
                higher_priorities = sorted({p for p in order_priority_map.values() if p < priority and p not in cascaded_priorities})
                        ...
                        if rescheduled_count_for_prio > 0:
                            with db_session:
                                new_rescheduled_items = select(ri for ri in RescheduledItem 
                                                               ...
                                                               (ri.operation.id, ri.order.id) not in processed_keys).distinct()[:]
    ...
    print(f"Rescheduling complete: {successful_reschedules} successful, {failed_reschedules} failed, {default_machine_skips} default machines skipped")
